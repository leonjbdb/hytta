...
// Demo deploys go to their own worker name so they never overwrite the `hytta`
// production worker. (The non-demo path leaves the source `name` untouched.)
function renameDemoWorker(config: string, sourcePath: string): string {
  const renamed = config.replace(/("name"\s*:\s*")hytta(")/, "$1hytta-demo$2");
  if (renamed === config) {
    throw new Error(`Could not rename the worker to hytta-demo in ${sourcePath}.`);
  }
  return renamed;
}
...
function writeDemoAppConfig(sourcePath: string, generatedPath: string): string {
  ...
  const generated = [
    ...
    "// Do not edit or commit this file.",
    renameDemoWorker(stripDemoAppBindings(source, sourcePath), sourcePath),
  ].join("\n");
