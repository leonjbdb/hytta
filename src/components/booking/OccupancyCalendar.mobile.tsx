...
export function OccupancyCalendar(props: OccupancyCalendarProps) {
  ...
  const dayPicker =
    selection.mode === 'range' ? (
      <DayPicker
        ...
        className={cn('rdp-root mx-auto w-full max-w-sm', rangeIncomplete && 'hytta-range-preview')}
  ...
  return (
    <OccupancyContext.Provider value={occupancyContext}>
      <div className="flex flex-col gap-3 rounded-xl border border-[var(--border)] bg-[var(--hytta-calendar-bg)] p-3">
        <CalendarHeader
          month={month}
          onMonthChange={setMonth}
          locale={locale}
...
function CalendarHeader({
  ...
  // browsable (`includePastYears`). Other years stay reachable by typing into
  // the field (`allowCustom`).
