...
export const { handlers, auth, signIn, signOut } = NextAuth(() => ({
  ...
  providers: [
    /*
     * Magic-link provider. Auth.js owns the verificationToken row lifecycle
     * (mint, single-use consume, expire). We override `sendVerificationRequest`
     * twice over: (a) gate on user existence so non-members never receive a
     * link — closes the "received-but-rejected" enumeration leak — and (b)
     * route the rendered template through the project's `mailer` facade so
     * Resend stays the single transport.
     */
    Nodemailer({
      // Unused: real send happens via mailer.sendMagicLink below.
      server: {},
      from: env.EMAIL_FROM,
      ...
      sendVerificationRequest: async ({ identifier, url, expires }) => {
        ...
        await mailer.sendMagicLink(identifier, await withRequestOrigin(url), locale, expires);
      },
    }),
    /*
     * DB-backed password sign-in. Hidden behind a tiny link on /login → the
     * primary path is magic-link. Returns null on any failure so Auth.js
     * surfaces a generic "CredentialsSignin" error — no enumeration of which
     * users have passwords vs. which use magic-link only.
     */
    Credentials({
      name: 'password',
      credentials: {
