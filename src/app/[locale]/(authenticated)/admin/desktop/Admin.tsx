...
function UsersSection({
  ...
  // Kicking is high-stakes and irreversible — force a 3-second wait even for
  // admins (who normally confirm instantly).
        ...
        <CardContent className="flex flex-col gap-2 py-4">
          ...
          {filtered.map((u) => {
            ...
            return (
              <div
                key={u.id}
                className="flex flex-wrap items-center justify-between gap-3 rounded-md bg-[var(--muted)]/30 p-3"
              >
                ...
                <div className="flex w-full items-center gap-2 sm:w-auto">
                  <div className="grid flex-1 grid-cols-3 gap-2 sm:w-[21rem] sm:flex-none">
                    <RoleToggle
                      label={t('roleAdmin')}
                      active={u.isAdmin}
                  ...
                  {u.id !== viewerId ? (
                    <button
                      ...
                      onClick={() => kick(u)}
                      className="inline-flex size-9 shrink-0 cursor-pointer items-center justify-center rounded-md text-[var(--destructive)] transition-colors hover:bg-[color-mix(in_oklch,var(--destructive),transparent_85%)] disabled:cursor-not-allowed disabled:opacity-50"
                    >
                      {kicking === u.id ? (
